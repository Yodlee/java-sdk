...
public class DerivedApiSdkTest {
	...
	public static void TestGetTransactionSummary(DerivedApi derivedApi, String testCaseId, String testCaseName,
		...
		try {
			...
			ApiResponse<DerivedTransactionSummaryResponse> derivedResponse =
					derivedApi.getTransactionSummary(GroupType.valueOf(groupBy), accountIds.toArray(new Long[0]), categoryIds.toArray(new Long[0]),
	...
	public static void TestGetHoldingSummary(DerivedApi derivedApi, String testCaseId, String testCaseName,
		...
		try {
			List<Long> accountIds = new ArrayList<Long>();
			ApiResponse<DerivedHoldingSummaryResponse> derivedResponse =
					derivedApi.getHoldingSummary(accountIds.toArray(new Long[0]), classificationType, DerivedInclude.valueOf(include));
			assertEquals(derivedResponse.getStatusCode(), 200);
			assertNotNull(derivedResponse.getData());
	...
	public static void TestGetNetworth(DerivedApi derivedApi, String testCaseId, String testCaseName, String accountId,
		...
		try {
			...
			ApiResponse<DerivedNetworthResponse> derivedResponse = derivedApi.getNetworth(accountIds.toArray(new Long[0]), fdate,
					DerivedInclude.valueOf(include), DataPointInterval.valueOf(interval), skip, tdate, top);
			assertEquals(derivedResponse.getStatusCode(), 200);
			assertNotNull(derivedResponse.getData());
		} catch (ApiException | ParseException e) {
			PrintUtil.assertFail(testCaseId, testCaseName, e);
